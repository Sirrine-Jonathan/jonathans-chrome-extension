...
console.log(
  "%c-->\tJonathan's Chrome Extension is active",
...
const reformatJson = () => {
  ...
  if (onlyPreTag) {
    ...
    try {
      ...
      console.error(
        new Error(`Failed to json parse content of page with only one pre tag`)
      );
    }
    try {
      ...
      console.error(
        new Error(
          `Failed to json stringify content of page with only one pre tag`
        )
      );
    }
